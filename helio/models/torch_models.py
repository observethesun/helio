...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, **kwargs):
        ...
        self.conv = ConvBlock('cacad', 3*out_channels, out_channels, **kwargs)
...
class Unet(nn.Module):
    ...
    def __init__(self, in_channels, depth, init_filters, kernel_size=3, output=None):
        ...
        for d in range(depth):
            ...
            self.down.append(Down('caca', in_channels, out_channels, kernel_size=kernel_size))
            self.down.append(Down('pd'))
            in_channels = out_channels

        self.bottom = ConvBlock('cacad', in_channels, init_filters * (2 ** depth), kernel_size=kernel_size)
